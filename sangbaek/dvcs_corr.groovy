  ...
  def processEvent(event){
    ...
    if (event.npart>0) {
      ...
      if(ele!=null) {
        ...
        hists.computeIfAbsent("/epg/pid/ele/pcalecaldep_S"+ele_sec, h_pcalecal).fill(pcal, eidep+eodep)
        ...
        if (DVCS.KineCuts(xB, Q2, W, ele, gam) || (event.mc_status && gam.e()>0.4)){
          ...
          if (DVCS.ExclCuts(gam, ele, VMISS, VmissP, VmissG, Vhadr, Vhad2)){
            ...
            hists.computeIfAbsent("/excl/pid/ele/pcalecaldep_S"+ele_sec, h_pcalecal).fill(pcal, eidep+eodep)
            ...
            if (number_of_photons>1){
              ...
              pi0 = gam+gam2
              hists.computeIfAbsent("/dvcs/pi0/after_corr/number_of_photons_gam1_energy", h_second_photons).fill(gam.e(), number_of_photons)
              hists.computeIfAbsent("/dvcs/pi0/after_corr/number_of_photons_gam2_energy", h_second_photons).fill(gam2.e(), number_of_photons)              
              ...
              if (pi0.mass()<0.2 && pi0.mass()>0.08)  {
                ...
                hists.computeIfAbsent("/dvcs/pi0/after_corr/kin_corr/h_t", h_t).fill(t_pi0)
                hists.computeIfAbsent("/dvcs/pi0/after_corr/kin_corr/h_gam_energy_corr_4vec", h_gam_energy_corr).fill(pi0.e(), nu + t/2/M)
                hists.computeIfAbsent("/dvcs/pi0/after_corr/kin_corr/h_gam_energy_corr_virtual", h_gam_energy_corr).fill(pi0.e(), nu + t_pi0/2/M)
                hists.computeIfAbsent("/dvcs/pi0/after_corr/kin_corr/h_gam_energy_corr_diff_4vec", h_gam_energy_corr_diff).fill(pi0.e(), nu + t/2/M - pi0.e())
